...
/*$resultat = mysql_query("SELECT score FROM ".$PA["table"]." WHERE x=$x");


do {
$cont = $cont + $mirc["score"];
}while ($mirc = mysql_fetch_array($resultat));*/




 $id = mysql_query("SELECT x,score FROM pa_users;");
 $i = 1;
 ...
 while($row = mysql_fetch_array($id)) {
  ...
  $continent[$row[0]] = $continent[$row[0]]+$row[1];

  if($row[0] > $i) { $i = $row[0]; }
 }
 mysql_query("DELETE from topcontinent;");
for($x = 0;$x < $i;$x++) {

$resultat = mysql_query("SELECT score, galname FROM ".$PA["table"]." WHERE x=$x");
do {
$cont = $cont + $mirc["score"];
$gal = $mirc["galname"];
}while ($mirc = mysql_fetch_array($resultat));

   /*mysql_query("INSERT INTO topcontinent (x,score) VALUES ('$x','$row[$x]')");*/
if ($gal != "")
{
mysql_query("INSERT INTO topcontinent (x,score,galname) VALUES ('$x','$cont','$gal')");
$cont=0;
}
...
mysql_query("DELETE from topcontinent where x=0;");


if ($tickdif<$tickertime-0 && $override!="true") die("Less than $tickertime seconds since last tick!");
...
$result = mysql_query("UPDATE ".$PA['table']." SET c_energy=c_energy-1 WHERE c_energy>1",$db); dbr(mysql_error());

$result = mysql_query("UPDATE ".$PA['table']." SET lastsleep=lastsleep-1 WHERE lastsleep>0",$db); dbr(mysql_error());
$result = mysql_query("UPDATE ".$PA['table']." SET sleep=sleep-1 WHERE sleep>0",$db); dbr(mysql_error());
$result = mysql_query("UPDATE ".$PA['table']." SET newbie=newbie-1 WHERE newbie>0",$db); dbr(mysql_error());
...
$result = mysql_query("UPDATE ".$PA['table']." SET r_energy=r_energy-1 WHERE r_energy>1",$db); dbr(mysql_error());
...
/*
    if ($c_crystal==1 && $r_imcrystal!=1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET crystal=crystal+".($krystall1+$crystalroid*80)." WHERE id=$id",$db); dbr(mysql_error());}
    if ($c_crystal==1 && $r_imcrystal==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET crystal=crystal+".($krystall2+$crystalroid*80)." WHERE id=$id",$db); dbr(mysql_error());}
...
$sivile = 0;
$sivile = floor(($civilians/12) * (1.1));
$inntekt=0;
$inntekt=$civilians * $tax / 100;
$inntekt = floor($inntekt);
$maksbeboere = 0;
$maksbeboere = $crystalroid * 300;

$result2 = mysql_query("UPDATE ".$PA["table"]." SET civilians=civilians+".($sivile)." WHERE id=$id AND $civilians<$maksbeboere",$db); dbr(mysql_error());

/*$result2 = mysql_query("UPDATE ".$PA["table"]." SET civilians=civilians-".($sivile)." WHERE id=$id AND civilians>$maksbeboere AND civilians < 3000",$db); dbr(mysql_error());*/

   
$ekstra = floor($inntekt*0.1);
 if ($c_crystal==1 && $r_imcrystal!=1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET crystal=crystal+".($inntekt)." WHERE id=$id",$db); dbr(mysql_error());}
    if ($c_crystal==1 && $r_imcrystal==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET crystal=crystal+".($inntekt+$ekstra)." WHERE id=$id",$db); dbr(mysql_error());}



 
if ($c_energy==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET energy=energy+".($sats*45)." WHERE id=$id",$db); dbr(mysql_error());}


$ekstra2 = floor($metalroid*0.1);
    if ($c_metal==1 && $r_immetal!=1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET metal=metal+".($metall1+$metalroid*60)." WHERE id=$id",$db); dbr(mysql_error());}
    if ($c_metal==1 && $r_immetal==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET metal=metal+".($metall2+$metalroid*60+$ekstra)." WHERE id=$id",$db); dbr(mysql_error());}

    if ($p_infinityst==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET p_infinityst='0',p_infinitys='0',infinitys_base=infinitys_base+$p_infinitys WHERE nick='$nick'",$db); dbr(mysql_error());}

    if ($p_warfrigst==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET p_warfrigst='0',p_warfrigs='0',warfrigs_base=warfrigs_base+$p_warfrigs WHERE nick='$nick'",$db); dbr(mysql_error());}

    if ($p_wraithst==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET p_wraithst='0',p_wraiths='0',wraiths_base=wraiths_base+$p_wraiths WHERE nick='$nick'",$db); dbr(mysql_error());}

    if ($p_astropodst==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET p_astropodst='0',p_astropods='0',astropods_base=astropods_base+$p_astropods WHERE nick='$nick'",$db); dbr(mysql_error());}

    if ($p_destroyerst==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET p_destroyerst='0',p_destroyers='0',destroyers_base=destroyers_base+$p_destroyers WHERE nick='$nick'",$db); dbr(mysql_error());}

    if ($p_cobrast==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET p_cobrast='0',p_cobras='0',cobras=cobras+$p_cobras WHERE nick='$nick'",$db); dbr(mysql_error());}

    if ($p_scorpionst==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET p_scorpionst='0',p_scorpions='0',scorpions_base=scorpions_base+$p_scorpions WHERE id=$id",$db); dbr(mysql_error());}

    if ($p_rcannonst==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET p_rcannonst='0',p_rcannons='0',rcannons=rcannons+$p_rcannons WHERE nick='$nick'",$db); dbr(mysql_error());}

    if ($p_avengerst==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET p_avengerst='0',p_avengers='0',avengers=avengers+$p_avengers WHERE nick='$nick'",$db); dbr(mysql_error());}

    if ($p_lstalkerst==1) {$result2 = mysql_query("UPDATE ".$PA["table"]." SET p_lstalkerst='0',p_lstalkers='0',lstalkers=lstalkers+$p_lstalkers WHERE nick='$nick'",$db); dbr(mysql_error());}
...
# Prepare defending fleets
        ...
        if ($myrow['war']==0 && $myrow['def']==0){
            $d_infinitys = $myrow['infinitys'] + $myrow['infinitys_base'];
            $d_wraiths = $myrow['wraiths'] + $myrow['wraiths_base'];
            $d_warfrigs = $myrow['warfrigs'] + $myrow['warfrigs_base'];
            $d_destroyers = $myrow['destroyers'] + $myrow['destroyers_base'];
            $d_cobras = $myrow['cobras'] + $myrow['cobras_base'];
            $d_scorpions = $myrow['scorpions'] + $myrow['scorpions_base'];
            $d_astropods = $myrow['astropods'] + $myrow['astropods_base'];
            $d_rcannons = $myrow['rcannons'];
            $d_avengers = $myrow['avengers'];
            $d_lstalkers = $myrow['lstalkers'];

            $defend[0]["id"] = $myrow['id'];
            $defend[0]["infinitys"] = $myrow['infinitys'] + $myrow['infinitys_base'];
            $defend[0]["wraiths"] = $myrow['wraiths'] + $myrow['wraiths_base'];
            $defend[0]["warfrigs"] = $myrow['warfrigs'] + $myrow['warfrigs_base'];
            $defend[0]["destroyers"] = $myrow['destroyers'] + $myrow['destroyers_base'];
            $defend[0]["cobras"] = $myrow['cobras'];
            $defend[0]["scorpions"] = $myrow['scorpions'] + $myrow['scorpions_base'];
            $defend[0]["astropods"] = $myrow['astropods'] + $myrow['astropods_base'];
            $defend[0]["sum"] = $myrow['infinitys'] + $myrow['infinitys_base'] + $myrow['wraiths'] + $myrow['wraiths_base']+ $myrow['warfrigs'] + $myrow['warfrigs_base'] + $myrow['destroyers']  + $myrow['destroyers_base'] + $myrow['cobras'] + $myrow['cobras_base'] + $myrow['scorpions']  + $myrow['scorpions_base'] + $myrow['astropods'] + $myrow['astropods_base'];
            $d_sum = $defend[0]["sum"];
        } else {
            ...
            $d_cobras = $myrow['cobras']; /* Cobras do  defend*/
            $d_scorpions = $myrow['scorpions_base'];
...
/*Her var det null */
            ...
            $defend[0]["destroyers"] = $myrow['destroyers_base'];
            $defend[0]["cobras"] = $myrow['cobras']; /* Cobras do  defend*/
...
# power of infinitys to 0.01 against wraiths.
        ...
        $stats['wra']['cob'] = 0.005 / 1.2;
        $stats['wra']['lst'] = 0.07 / 1.2;

        $stats['war']['inf'] = 2.5 / 1.2;
...
<td>Soldiers:</td><td>$d_infinitys2</td><td>".(number_format($inf,0))."<tr>\n
<td>Navy Seals:</td><td>$d_wraiths2</td><td>".(number_format($wra,0))."<tr>\n
<td>Warfigates:</td><td>$d_warfrigs2</td><td>".(number_format($war,0))."<tr>\n
<td>Destroyers:</td><td>$d_destroyers2</td><td>".(number_format($des,0))."<tr>\n
<td>Anti Thiefs:</td><td>$d_cobras2</td><td>".(number_format($cob,0))."<tr>\n
<td>Thiefs:</td><td>$d_astropods2</td><td>".(number_format($ast,0))."<tr>\n
<td>Scorpions:</td><td>$d_scorpions2</td><td>".(number_format($sco,0))."<tr>\n
...
<td>Thiefs:</td><td>$astropods2 ($block blocked)</td><td>".(number_format($astropods2-$astropods,0))."<tr>\n
<td>Scorpions:</td><td>$scorpions2</td><td>".(number_format($scorpions2-$scorpions,0))."<tr>\n
<td>Yours:</td><tr>\n
<td>Soldiers:</td><td>".$attack[$i]["infinitys"]."</td><td>$l_infinitys<tr>\n
<td>Navy Seals:</td><td>".$attack[$i]["wraiths"]."</td><td>$l_wraiths<tr>\n
<td>Warfrigates:</td><td>".$attack[$i]["warfrigs"]."</td><td>$l_warfrigs<tr>\n
<td>Destroyers:</td><td>".$attack[$i]["destroyers"]."</td><td>$l_destroyers<tr>\n
<td>Thiefs:</td><td>".$attack[$i]["astropods"]." ($block blocked)</td><td>$l_astropods<tr>\n
<td>Scorpions:</td><td>".$attack[$i]["scorpions"]."</td><td>$l_scorpions<tr>\n

<tr>
<td>Roids captured:</td><tr>
<td>Silver:</td><td>".$roidsc["metal"]."/$d_metalroid</td><tr>
<td>Gold:</td><td>".$roidsc["crystal"]."/$d_crystalroid</td><tr>
<td>Undeveloped:</td><td>".$roidsc["ui"]."/$d_ui_roids</td><tr>
</table>\n
",$attack[$i]["id"]);
...
/*$result3 = mysql_query("UPDATE ".$PA["table"]." SET infinitys=infinitys-$inf,wraiths=wraiths-$wra,warfrigs=warfrigs-$war,destroyers=destroyers-$des,astropods=astropods-$l_astropods,scorpions=scorpions-$sco WHERE id=".$attack[$i]["id"],$db);  dbr(mysql_error());*/            
$i++;
...
/*$mistet = $civilians * 200;
$result3 = mysql_query("UPDATE ".$PA["table"]." SET civilians=civilians - $mistet WHERE id=".$attack[$i]["id"],$db);  dbr(mysql_error());*/
        }
...
<td>Navy Seals:</td><td>$d_wraiths2</td><td>".(number_format($d_wraiths2-$d_wraiths,0))."<tr>\n
...
<td>Anti Thiefs:</td><td>".$defend[$i]["cobras"]."</td><td>$l_cobras<tr>\n
...
$result3 = mysql_query("UPDATE ".$PA["table"]." SET infinitys_base=infinitys_base-$inf,wraiths_base=wraiths_base-$wra,warfrigs_base=warfrigs_base-$war,destroyers_base=destroyers_base-$des,astropods_base=astropods_base-$ast,scorpions_base=scorpions_base-$sco WHERE id=".$defend[0]["id"],$db);  dbr(mysql_error());
...
$result3 = mysql_query("UPDATE ".$PA["table"]." SET infinitys_base=0,wraiths_base=0,warfrigs_base=0,destroyers_base=0,astropods_base=0,scorpions_base=0,cobras_base=0 WHERE infinitys_base < 0 AND wraiths_base < 0 AND warfrigates_base < 0 AND astropods_base < 0 AND cobras < 0 AND scorpions_base < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());
$result3 = mysql_query("UPDATE ".$PA["table"]." SET infinitys=0 where infinitys < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET infinitys_base=0 where infinitys_base < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());


$result3 = mysql_query("UPDATE ".$PA["table"]." SET cobras=0 where cobras < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET wraiths=0 where wraiths < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET wraiths_base=0 where wraiths_base < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET warfrigs=0 where warfrigs < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET warfrigs_base=0 where warfrigs_base < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET destroyers=0 where destroyers < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET destroyers_base=0 where destroyers_base < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());
$result3 = mysql_query("UPDATE ".$PA["table"]." SET scorpions=0 where scorpions < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());
$result3 = mysql_query("UPDATE ".$PA["table"]." SET scorpions_base=0 where scorpions_base < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET astropods=0 where astropods < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());
$result3 = mysql_query("UPDATE ".$PA["table"]." SET astropods_base=0 where astropods_base < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());
...
$result3 = mysql_query("UPDATE ".$PA["table"]." SET avengers=0 where avengers < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET rcannons=0 where rcannons < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET lstalkers=0 where lstalkers < 0 AND id=".$defend[$i]["id"],$db);  dbr(mysql_error());
...
$result3 = mysql_query("UPDATE ".$PA["table"]." SET score=(crystal+metal+energy)/200+size*1500+infinitys*250+infinitys_base*250+wraiths*1000+wraiths_base*1000+warfrigs*5000+warfrigs_base*5000+astropods*1600+astropods_base*1600+cobras*3000+cobras_base*3000+destroyers*5000+destroyers_base*5000+scorpions*7000+scorpions_base*7000+rcannons*800+avengers*1400+lstalkers*6000");


$result3 = mysql_query("UPDATE ".$PA["table"]." SET infinitys=0 where infinitys < 0",$db);  dbr(mysql_error());
$result3 = mysql_query("UPDATE ".$PA["table"]." SET cobras=0 where cobras < 0",$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET infinitys_base=0 where infinitys_base < 0",$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET wraiths=0 where wraiths < 0",$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET wraiths_base=0 where wraiths_base < 0",$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET warfrigs=0 where warfrigs < 0",$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET warfrigs_base=0 where warfrigs_base < 0",$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET destroyers=0 where destroyers < 0",$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET destroyers_base=0 where destroyers_base < 0",$db);  dbr(mysql_error());
$result3 = mysql_query("UPDATE ".$PA["table"]." SET scorpions=0 where scorpions < 0",$db);  dbr(mysql_error());
$result3 = mysql_query("UPDATE ".$PA["table"]." SET scorpions_base=0 where scorpions_base < 0",$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET astropods=0 where astropods < 0",$db);  dbr(mysql_error());
$result3 = mysql_query("UPDATE ".$PA["table"]." SET astropods_base=0 where astropods_base < 0",$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET avengers=0 where avengers < 0 ",$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET rcannons=0 where rcannons < 0 ",$db);  dbr(mysql_error());

$result3 = mysql_query("UPDATE ".$PA["table"]." SET lstalkers=0 where lstalkers < 0 ",$db);  dbr(mysql_error());
...
<?
if ($os=="windows") echo "<meta HTTP-EQUIV=\"Refresh\" content=\"60\">";
