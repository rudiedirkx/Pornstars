...
$gal=mysql_query("select count(nick) as peeps from pa_users where x='$x'  ");
...
mysql_query("update pa_users set commander='2' where x='$x'  and nick='$choosemowgq'"); } 

if (($choosemocgq != "") && ($choosemocgq != "None")) { mysql_query("update pa_users set commander='0' where x='$x' and y='$y' and commander='3'"); 
mysql_query("update pa_users set commander='3' where x='$x' and nick='$choosemocgq'"); } 
...
{
echo "voted is $voted";
$voterold=mysql_query("select vote from pa_users where x='$x'  group by vote order by x desc LIMIT 1");
$voteqold=mysql_fetch_object($voterold);
 mysql_query("update pa_users set vote=$voted where id=$Userid");
}
*/
...
$voteqold=mysql_fetch_object($voterold);
mysql_query("update pa_users set vote='$voted' where id='$Userid'");
$voteq=mysql_query("select vote from pa_users where x='$x'  and vote<>'' group by vote order by x desc LIMIT 1");
...
else { mysql_query("update pa_users set commander='0' where x='$x' "); }
 } 

$query=mysql_query("select * from pa_users where x='$x'  order by y");
echo("<center><table border='1' width='62%'>");
echo("<tr><td width='4%'>X</td><td width='4%'>Y</td><td width='30%'>Nick</td><td width='30%'>Voted on</td><td width='10%'>Votes</td></tr>");
while ($record=mysql_fetch_object($query)) { 

$vote=mysql_query("select vote from pa_users where vote='$record->nick' group by vote");
...
echo("<tr><td width='4%'>$record->x</td><td width='4%'>$record->y</td><td width='30%'>$kleur$record->nick</td><td width='30%'>$record->vote&nbsp;</td><td width='10%'>$votes->vote&nbsp;</td></tr>"); 
$kleur="<font color=white>";
}

echo("</table>"); 

$query2=mysql_query("select * from pa_users where x='$x'  order by y");
echo("<form method='post' action=senate2.php>");echo("<br><select size='1' name='voted'>");
...
$query2=mysql_query("select * from pa_users where x='$x'  and commander<>'1' order by y");
